# fireassay — milestone log (M1 → M3b)

What each milestone built, what it deliberately did not, and the corrections applied to
the original specs while building. This is the historical record the README used to open
with (moved here 2026-09-10 so the README can lead with what was measured). The per-milestone
specs are `M1-SPEC.md` … `M3b-SPEC.md` and `M-ITEMS-SPEC.md` at the repo root; the parent
spec is `docs/SPEC.md`. Nothing below has been edited in the move except this preamble.
